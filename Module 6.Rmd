...
par(mfrow = c(2, 3))
attach(d)
boxplot(d$population, na.rm = TRUE)
boxplot(log(d$population), na.rm = TRUE)
boxplot(d$area, na.rm = TRUE)
boxplot(log(d$area), na.rm = TRUE)
barplot(d$population, na.rm = TRUE)
barplot(d$area, na.rm = TRUE)
detach(d)
```
...
hist(log(area), freq = FALSE, col = "blue", main = "Plot 2", xlab = "log(area)",
    ylab = "density", ylim = c(0, 0.2))
 abline(v=mean(log(population))) # I don't see the difference. Supposed to add mean pop line
...
head(n)
summary(n)
boxplot(log(Body_mass_female_mean) ~ Family, n)


library(ggplot2)
p <- ggplot(data = n, aes(x = Family, y = log(Body_mass_female_mean)))  #define the variables
